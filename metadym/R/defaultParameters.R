...
getDefaultParameters <- function(){
  ...
  # s_c : matrix of dimension D X R with niche breadth for each species and environmental variable for colonization
  
  s_c = matrix(nr = D, nc = R)
  s_c[1,] = rep(0.3, R)
  s_c[2,] = rep(10, R)
  
  # s_e : matrix of dimension D X R with niche breadth for each species and environmental variable for extinction
  
  s_e = matrix(nr = D, nc = R)
  s_e[1,] = rep(0.3, R)
  s_e[2,] = rep(10, R)
  ...
  pars = list(u_c = u_c, 
              ...
              s_c = s_c, 
              s_e = s_e, 
              alpha = 0.1, # alpha : scalar, average dispersal distance
              m = 0.001, # m : scalar, immigration probability
              
              c_0 = rep(2, R), 
              e_0 = rep(0.02, R), # e_0 : vector of dimension R with extinction probability at 0 interaction
